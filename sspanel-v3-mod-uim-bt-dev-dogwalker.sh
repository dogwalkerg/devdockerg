...
wget -N --no-check-certificate "https://raw.githubusercontent.com/dogwalkerg/dogwalkerdockerg/master/dogwalker.zip"
unzip dogwalker.zip
cd ss-panel-v3-mod_Uim-dogwalker
mv * .[^.]* /www/wwwroot/$website/
cd ..
rm -rf dogwalker.zip ss-panel-v3-mod_Uim-dogwalker/
...
echo -e "${Info} 正在导入数据库"
cd sql/
mysql -u$mysqlusername -p$mysqlpassword $mysqldatabase < glzjin_all.sql >/dogwalker/null 2>&1
...
cd /www/wwwroot/$website
cp config/.config.php.dogwalker config/.config.php
